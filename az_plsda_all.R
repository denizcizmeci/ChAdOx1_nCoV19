...
pdf("feature_selection_repetition_figureS2.pdf", width = 10, height = 4)
print(plt_bar)
dev.off()
...
# save(vals_Figure3_v2, file = "vals_FigureS2.RData")
# 
# cv.acc <- 0
# for (i in 1:100){
...
# 
# 
# pdf("vals_FigureS2_val.pdf", width = 3, height = 3)
# plt_val
# dev.off()
...
pdf("FigureS2AB_plsda_lasso90.pdf", width = 7, height = 5)
p.scores  + p.loads 
#+ plt_val
dev.off()
...
pdf("FigureS2AB_plsda_lasso90_vip.pdf", width = 7, height = 5)
p.scores  + p.vip
#+ plt_val
dev.off()
...
pdf(paste0("FigureS2AB_cornet.pdf"), width = 14, height = 14)
...
pdf("FigureS2AB_lasso_uni.pdf", width = 10, height = 10)
ggplot(temp, aes(group, log10(value)))+
  ...
  ylim(3.3,6.3)+
  xlab(" ") +ylab("log10 MFI")+ facet_wrap(.~feature, ncol = 3, scales = "free_x")
dev.off()
